...
from nav2_msgs.msg import SpeedLimit  # <--- use max speed from nav2 config
import math 
...
class StartInspection(Node):
    def __init__(self):
        ...
        # state sequence by default obj_1 ( With state machine following the sequence pick place)
        ...
        self.GOAL3_X = 0.4727249727945442
        self.GOAL3_Y = 4.481985477209297
        self.GOAL3_Z = 0.0

        self.GOAL3_ORI_Z = 0.186616765327272
        self.GOAL3_ORI_W = 0.9824327879803207

        # orienatation differnce set to higher value       
        self.rz_3 = self.rw_3 = 1.0

        self.return_intermediate_reached = False 


        self.safezone_reached = False
        ...
        # create the action client, motion controller by moveit (Will use to close and open gripper to full capacity)
        ...
        # open gripper , this function use playmotion2
        self.send_gripper_command("open")
    ...
    # Before move manually, calculate the distance between where you are and where you want to be.
    ...
    def laser_scan_callback(self, msg):
        # Check for open space in front of the robot 

        # STEP 1: If we have ALREADY detected the door is open, NEVER run this again!
        ...
        if min_distance > 1.5:  # Adjust this threshold as needed
            self.door_is_open = True #This permanently locks the laser scanner from hijacking Nav2
    ...
    # result callback for pick table goal .
...
# navigation check function 
    ...
    # In below function we check the robot angular speed
    ...
    def _goal1_result_callback(self, future):
        ...
        try:
            ...
            # calculating error using the below formula
...
#################################GOAL 3  Intermediate return psotion#######################################################

# basically this goal3 is used when object is place and arm is in safe position returning back to pick table. The intermediate pose Goal 3 is used 

    def navigate_to_goal3(self):
        """Navigate to the place table (goal 2)"""
        self.get_logger().info("=" * 50)
        self.get_logger().info("NAVIGATING TO PLACE TABLE (GOAL 3 return intermediate)")
        self.get_logger().info("=" * 50)
        ...
        # Goal 2 position
        goal.pose.position.x = self.GOAL3_X
        goal.pose.position.y = self.GOAL3_Y
        goal.pose.position.z = self.GOAL3_Z
        
        # Goal 2 orientation
        goal.pose.orientation.z = self.GOAL3_ORI_Z
        goal.pose.orientation.w = self.GOAL3_ORI_W
        
        # Publish the goal
        self.goal_publisher_.publish(goal)
        self.get_logger().info(f'Published Goal 3: x={goal.pose.position.x:.3f}, y={goal.pose.position.y:.3f}')
        ...
        # Send the goal
        self._send_goal_future = self._action_client.send_goal_async(goal_msg)
        self._send_goal_future.add_done_callback(self._goal3_response_callback)
    ...
    def _goal3_response_callback(self, future):
        """Handle Goal 1 acceptance"""
        goal_handle = future.result()
        if not goal_handle.accepted:
            self.get_logger().error('Goal 3 RETURN was rejected!')
            return

        self.get_logger().info('Goal 3 RETURN was accepted! Moving to place table...')
        ...
        # Wait for result
        self._get_result_future = goal_handle.get_result_async()
        self._get_result_future.add_done_callback(self._goal3_result_callback)

    def _goal3_result_callback(self, future):
        """Handle Goal 3 completion"""
        try:
            result = future.result()
            status = result.status
            
            self.get_logger().info(f"Goal 3 navigation finished with status: {status}")
            ...
            # Calculate error
            dx = self.GOAL3_X - self.current_pose.position.x
            dy = self.GOAL3_Y - self.current_pose.position.y
            error = math.sqrt(dx**2 + dy**2)

            self.rz_3 = abs(self.GOAL3_ORI_Z - self.current_pose.orientation.z) # taking absolute value
            self.rw_3 = abs(self.GOAL3_ORI_W - self.current_pose.orientation.w) # taking absolute value
           ...
           # Printing Logs: tolerance, current and target pose for position and orientation
            self.get_logger().info(
                f"\n"
                f"===== GOAL 3 POSITION CHECK =====\n"
                f"Target:   ({self.GOAL3_X:.3f}, {self.GOAL3_Y:.3f})\n"
                f"Current:  ({self.current_pose.position.x:.3f}, {self.current_pose.position.y:.3f})\n"
                f"Error: {error:.3f} m\n"
                f"Tolerance: {tolerance:.3f} m\n"
                f"Target_rotation:   ({self.GOAL3_ORI_Z:.3f}, {self.GOAL3_ORI_W:.3f})\n"
                f"Current_rotation:  ({self.current_pose.orientation.z:.3f}, {self.current_pose.orientation.w:.3f})\n"
                f"Error_rotation:    ({self.rz_3:.3f}, {self.rw_3:.3f}) m\n"
                f"================================"
            )
  
            
            if error <= tolerance :
                self.get_logger().info("Reached Goal 3 return!")
                self.get_logger().info("robot stop!") 
                self.return_intermediate_reached = True 
                
            else:
                self.get_logger().warn(f" Not within tolerance (error: {error:.3f}m)")
                
        except Exception as e:
            self.get_logger().error(f"Error in Goal 3 result callback: {e}")
...
# collision object function
    ...
    # Below function add collision object from the marker 
    ...
    def collision_objects_pick(self):
       ...
       # collision 
        ...
        # position
        col_marker_table_1.pose.position.x = self.pos_TCx + 0.243 
        col_marker_table_1.pose.position.y = self.pos_TCy - 5*0.0162 
        col_marker_table_1.pose.position.z = self.pos_TCz - 0.052 
        #orientation- 
        ...
        col_marker_table_1.pose.orientation.z = self.rot_TCz
        col_marker_table_1.pose.orientation.w = self.rot_TCw 
    ...
    # Below function add arrow marker for visulaization wherw the robot arm will going to move
...
###############################OBJECT 2 #############################################
        ...
        # --- Object 2: Holdback ---
        self.holdback_obj_2_wp = Waypoint()
        ...
        self.holdback_obj_2_wp.pose = holdback_obj_2_pose
...
######################### OBJECT 3 ###############################
        ...
        pregrasp_obj_3_pose.pose.position.x = self.pos_03Cx - c
        pregrasp_obj_3_pose.pose.position.y = self.pos_03Cy - 0.01
        pregrasp_obj_3_pose.pose.position.z = self.pos_03Cz + a
        ...
        grasp_obj_3_pose.pose.position.x = self.pos_03Cx - b
        grasp_obj_3_pose.pose.position.y = self.pos_03Cy - 0.01
        grasp_obj_3_pose.pose.position.z = self.pos_03Cz + a
        ...
        lift_obj_3_pose.pose.position.x = self.pos_03Cx - b
        lift_obj_3_pose.pose.position.y = self.pos_03Cy - 0.01
        lift_obj_3_pose.pose.position.z = self.pos_03Cz + 3*a
        lift_obj_3_pose.pose.orientation.w = 1.0
        self.lift_obj_3_wp.pose = lift_obj_3_pose

        # --- Object 3: Holdback ---
        self.holdback_obj_3_wp = Waypoint()
        holdback_obj_3_pose = PoseStamped()
        holdback_obj_3_pose.header.frame_id = "base_link"
        holdback_obj_3_pose.pose.position.x = self.pos_03Cx - c
        holdback_obj_3_pose.pose.position.y = self.pos_03Cy - 0.01
        holdback_obj_3_pose.pose.position.z = self.pos_03Cz + 3*a
        holdback_obj_3_pose.pose.orientation.w = 1.0
        self.holdback_obj_3_wp.pose = holdback_obj_3_pose
      ...
      ############### object 3 marker ################
        ...
        #### THE BELOW CONDITION DEPENDS ON SEQUENCE, TO PUBLISH MARKER
    ...
    # The below function is the pick sequence, by using planning and execution service the arm moves to object and pick it 
...
############## Delete  marker for place objects ####
    ...
    # The TF function , loading pose information from AprilTags
    ...
    def get_tag_pose(self):
        ...
        # Tables + unknown object IDs ( Even if object id is changed it will work)
    ...
    ## Once the pose available , saving pose to perform task while sometime when arm comes in front of camera we loss apriltag pose
...
########################PLACE OBJECT#################################################
    ...
    # The below function content sub function to process place sequence
    ...
    def test_collision_object_place(self):
        ...
        # checking the flag whether reached table place (Self.secon_goal_reached is True--> for table place )
    ...
    # aRROW MARKER FOR VISULAIZATION
        ...
        if self.state_sequence == "obj_1":
            ...
            for attempt in range(1, MAX_RETRIES + 1):
                ...
                if plan_safezone and plan_safezone.success:
                    ...
                    self.remove_collision_obj(["table_2","obj_2"])
                    self.delete_visualization_markers2()
            ...
            for attempt in range(1, MAX_RETRIES + 1):
                ...
                if plan_safezone and plan_safezone.success:
                    ...
                    # ===========================================
                    self.state_sequence = "done"  # Tells the master loop to stop!
                    self.first_reached = False
                    self.intermediate_reached = False
                    self.second_goal_reached = False


                    self.remove_collision_obj(["table_2"])
                    self.delete_visualization_markers2()
...
def main():
    ...
    try:
        ...
        while rclpy.ok() and node.state_sequence in ["obj_1", "obj_2", "obj_3"]:
            node.get_logger().info(f"====== STARTING CYCLE FOR: {node.state_sequence} ======")
            rclpy.spin_once(node, timeout_sec=0.1)
        
            node.get_logger().info("Navigation goal sent!")

            # CRITICAL FIX 1: Reset all flags at the start of every cycle!
            node.first_reached = False
            node.intermediate_reached = False
            node.second_goal_reached = False
            node.return_intermediate_reached = False
            node.safezone_reached = False
            node.poses_latched = False
            node.robot_is_stopped = False
            ...
            if node.first_reached:
                ...
                # 🛑 CRITICAL FIX: Wipe the old memory so it takes a fresh picture!
                node.pos_TCx = 0.0
                node.pos_01Cx = 0.0
                node.pos_02Cx = 0.0
                node.pos_03Cx = 0.0
                node.latched_pose.clear()
            ...
            # check second goal the final goal reached or not (Table Place)
            ...
            node.get_logger().info("Sending Goal retun intermediate..")
    

            node.get_logger().info("Monitoring Goal 3...")
            goal3_tolerance = 0.35
            goal3_target_x = node.GOAL3_X
            goal3_target_y = node.GOAL3_Y

            while rclpy.ok() and not node.safezone_reached:
                rclpy.spin_once(node, timeout_sec=0.1)

                if node.current_pose is not None:
                    dx = goal3_target_x - node.current_pose.position.x
                    dy = goal3_target_y - node.current_pose.position.y
                    error = math.sqrt(dx**2 + dy**2)
                    node.get_logger().info(f"Goal 3 retunr intermediate error = {error:.3f}m")
                    if error <= goal3_tolerance:
                        node.return_intermediate_reached = True
                        node.get_logger().info("✅ GOAL 3 CONFIRMED — Sending Goal 2...")
                        node.safezone_reached = True
                        node.publish_goal()
                        break

# All collision object and valid markers. Red obj1 , blue obj2 , green obj 2
...
class MarkerVisualization(Node):
    ...
    def test_collision_object(self):
        ...
        #if self.pos_Tx != 0.0 and self.pos_01x != 0.0 and self.pos_02x != 0.0 and self.pos_03x :
        self.collision_objects()

        # all markers for position
        self.move_markers()

    def collision_objects(self,target_type):
        self.get_logger().info("Adding collision objects..")
       
        # 1. marker a object is created for the table 
        if target_type == "table_1":
        ...
        # create a list
        #markers = [col_marker_table_1, col_marker_obj_1, col_marker_obj_2, col_marker_obj_3, col_marker_table_2]
        

        # adding collision table at pick location
        if self.pos_Tx != 0.0 and target_type == "table_1":
        #markers = [col_marker_table_1, col_marker_obj_1]
            self.add_coll_objects_from_markers([col_marker_table_1])
            self.get_logger().info(f"Adding Pick Table Collision object  at: x={self.pos_TCx}, y={self.pos_TCy}, z={self.pos_TCz}")
        else:
        ...
        if self.pos_01x != 0.0 and target_type == "obj_1":
            self.add_coll_objects_from_markers([col_marker_obj_1])
            self.get_logger().info(f"Adding obj_1 at: x={self.pos_01Cx}, y={self.pos_01Cy}, z={self.pos_01Cz}")
        else :
            self.get_logger().info("no tag")

        # obj2
        
        if self.pos_02x != 0.0 and target_type == "obj_2":
            self.add_coll_objects_from_markers([col_marker_obj_2])
            self.get_logger().info(f"Adding obj_1 at: x={self.pos_02Cx}, y={self.pos_02Cy}, z={self.pos_02Cz}")
        else :
            self.get_logger().info("no tag")

        # obj3
        
        if self.pos_03x != 0.0 and target_type == "obj_3":
            self.add_coll_objects_from_markers([col_marker_obj_3])
            self.get_logger().info(f"Adding obj_1 at: x={self.pos_03Cx}, y={self.pos_03Cy}, z={self.pos_03Cz}")
        else :
            self.get_logger().info("no tag")

        # table place 
        
        if self.pos_TPx != 0.0 and target_type == "table_2":
            self.add_coll_objects_from_markers([col_marker_table_2])
            self.get_logger().info(f"Adding obj_1 at: x={self.pos_TPCx}, y={self.pos_TPCy}, z={self.pos_TPCz}")
        else :
            self.get_logger().info("no tag")
        
        # for visulaization publish to, visualization_marker topic
        #   self.publisher.publish(markers) 

        # adding collision_objects
    ...
    def move_markers(self):
        ...
        pregrasp_obj_1_pose.pose.position.x = self.pos_01Cx - 0.15 
        ...
        pregrasp_obj_1_pose.pose.orientation.w = 1.0 # 0.707
        ...
        grasp_obj_1_pose.pose.position.y = self.pos_01Cy # make shift left
        ...
        lift_obj_1_pose.pose.position.x = self.pos_01Cx 
        lift_obj_1_pose.pose.position.y = self.pos_01Cy
        lift_obj_1_pose.pose.position.z = self.pos_01Cz + 3*0.10 # 10cm higher + 10cm offset
        lift_obj_1_pose.pose.orientation.w = 0.1
        ...
        holdback_obj_1_pose.pose.position.x = self.pos_01Cx - 0.15
        holdback_obj_1_pose.pose.position.y = self.pos_01Cy
        holdback_obj_1_pose.pose.position.z = self.pos_01Cz + 3*0.10 # 10cm higher + 10cm offset
        holdback_obj_1_pose.pose.orientation.w = 0.1
...
###############################OBJECT 2 #############################################
        ...
        pregrasp_obj_2_pose.pose.position.x = self.pos_02Cx - 0.15 
...
######################### OBJECT 3 ###############################
        ...
        pregrasp_obj_3_pose.pose.position.x = self.pos_03Cx - 0.15 
        ...
        grasp_obj_3_pose.pose.position.x = self.pos_03Cx - 0.02
        ...
        lift_obj_3_pose.pose.orientation.w = 0.1
        ...
        holdback_obj_3_pose.pose.orientation.w = 0.1
    ...
    def get_tag_pose(self):
        ...
        # already discovered object IDs
        #tag_ids += list(self.object_slots.keys())
...
# self.object_slots.clear(), ONCE THE PICK PLACE CYCLE COMPLETE ADD THIS OTHERWISE OLD SLOT IS USED 
...
def main():
    ...
    # Track which objects have been successfully added to the MoveIt scene
    added_to_scene = {
        "table_1": False,
        "obj_1": False,
        "obj_2": False,
    ...
    try:
        node.get_logger().info("Continuous perception loop active...")
        
        while rclpy.ok():
            # 1. Update transforms from TF (get_tag_pose calls update_tag_pose)
            node.get_tag_pose()  
            
            # 2. Check each object. If found (pos != 0) and not yet added, add it once.
            if not added_to_scene["table_1"] and node.pos_Tx != 0.0:
                node.collision_objects("table_1")
                added_to_scene["table_1"] = True

            if not added_to_scene["obj_1"] and node.pos_01x != 0.0:
                node.collision_objects("obj_1")
                added_to_scene["obj_1"] = True
                
            if not added_to_scene["obj_2"] and node.pos_02x != 0.0:
                node.collision_objects("obj_2")
                added_to_scene["obj_2"] = True

            # ... repeat for obj_3 and table_2 ...

            # obj3 
            if not added_to_scene["obj_3"] and node.pos_03x != 0.0:
                node.collision_objects("obj_3")
                added_to_scene["obj_3"] = True

            # table 2

            if not added_to_scene["table_2"] and node.pos_TPx != 0.0:
                node.collision_objects("table_2")
                added_to_scene["table_2"] = True


            # 3. Spin once to allow TF buffer to fill and logs to print

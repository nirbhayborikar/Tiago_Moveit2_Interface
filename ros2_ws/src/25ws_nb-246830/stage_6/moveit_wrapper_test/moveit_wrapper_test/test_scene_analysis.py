...
def main():
    ...
    # Tracking dictionary to ensure we only call the service ONCE per object
    added_to_scene = {
        "table_1": False,
        "obj_1": False,
        "obj_2": False,
    ...
    try:
        node.get_logger().info("Perception loop started. Drive to locate tags...")
        
        while rclpy.ok():
            # 1. Update transforms (this updates pos_Tx, pos_TPx, etc.)
            node.get_tag_pose()  
            
            # 2. Check for Table 1 (Pick Table)
            if not added_to_scene["table_1"] and node.pos_Tx != 0.0:
                node.collision_objects("table_1")
                added_to_scene["table_1"] = True

            # 3. Check for Table 2 (Place Table) - This will trigger when you drive near it

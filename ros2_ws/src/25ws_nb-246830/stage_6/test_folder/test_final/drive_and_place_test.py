...
class StartInspection(Node):
    def __init__(self):
        ...
        self.GOAL2_ORI_Z = -0.0254971623464817 # 0.20
        self.GOAL2_ORI_W = 0.9996748945093485 # 0.30



        # high value so check will not pass
        self.rz = self.rw = 1.0
...
# navigation check function 
    ...
    def is_robot_stopped(self, linear_threshold=0.45, angular_threshold=0.20):
        """
    ...
    def _goal2_result_callback(self, future):
        ...
        try:
            ...
            self.rz = abs(self.GOAL2_ORI_Z - self.current_pose.orientation.z)
            self.rw = abs(self.GOAL2_ORI_W - self.current_pose.orientation.w)
            ...
            #and abs(rz)<= 0.03 and abs(rw)<= 0.03
...
# attach object
    ...
    def collision_objects(self):
        ...
        #orientation- 
        # col_marker_table_2.pose.orientation.x = self.rot_TPCx
        # col_marker_table_2.pose.orientation.y = self.rot_TPCy
        # col_marker_table_2.pose.orientation.z = self.rot_TPCw
        # col_marker_table_2.pose.orientation.w = self.rot_TPCy
...
#         node.publish_initial_pose()  # Call here 
...
#                     rz = node.GOAL2_ORI_Z - node.current_pose.orientation.z
#                     rw = node.GOAL2_ORI_W - node.current_pose.orientation.w
#                     error = math.sqrt(dx**2 + dy**2)
                    
#                     node.get_logger().info(f"Goal 2 error = {error:.3f}m (tolerance: {tolerance}m)")
                    
#                     if error <= tolerance and abs(rz)<= 0.03 and abs(rw)<= 0.03:
...
#         if tag_found and node.second_goal_reached == True:
...
def main():
    ...
    try:
        ...
        node.publish_initial_pose()  # This sends Goal 1 internally

        node.get_logger().info("Monitoring Goal 1...")
        #goal1_tolerance = 0.30
        ...
        while rclpy.ok() and not node.first_reached:
            ...
            if node.current_pose is not None:
                ...
                if error <= 0.35:
                    ...
                    node.navigate_to_goal2()  # Called ONCE from main, not from callback
                    break
...
# # and abs(rz) <= 0.03 and abs(rw) <= 0.03
#                 if error <= goal2_tolerance:



# # 2. Safely get the current rotational speed (default to 1.0 if not yet received so it doesn't accidentally pass)
#                     #if abs(rz) <= 0.08 and abs(rw) <= 0.08:
#                     node.get_logger().info("✅ cheching orientation  pose erroe need to be 0.08 less!")
#                     angular_z_speed = abs(node.current_velocity.angular.z) if node.current_velocity else 1.0
                    
#                     # 3. Check if the rotational speed is effectively zero
#                     if angular_z_speed <= 0.05 : # 0.158
#                         node.get_logger().info("✅ GOAL 2 REACHED AND ROTATION STOPPED!")
#                         time.sleep(0.3)
#                         node.second_goal_reached = True
#                        # time.sleep(0.05)
#                         # if angular_z_speed <= 0.05:
#                         #     node.get_logger().info("✅ GOAL 2 REACHED AND ROTATION STOPPED!")
#                         #     break
#                         # else:
#                         #     node.get_logger().info("✅ still rotating!")
                            
#                         break
#                     else:
#                         node.get_logger().info(f"At Goal 2 position, waiting for rotation to stop... (Speed: {angular_z_speed:.3f})", throttle_duration_sec=1.0)


#                 else:
#                     node.get_logger().info(f" {rz:.4f}, {rw:.4f},cheching orientation  pose!")
#                     node.get_logger().info("Going towards GOAL 2  !")
...
# ========================================================
        ...
        # 1. Add a counter before the loop starts
        stable_stop_count = 0  
        ...
        while rclpy.ok() and not node.second_goal_reached:
            ...
            if node.current_pose is not None:
                dx = node.GOAL2_X - node.current_pose.position.x
                dy = node.GOAL2_Y - node.current_pose.position.y
                rz = abs(node.GOAL2_ORI_Z - node.current_pose.orientation.z)
                rw = abs(node.GOAL2_ORI_W - node.current_pose.orientation.w)
                error = math.sqrt(dx**2 + dy**2)
                ...
                if error <= goal2_tolerance and rz<=0.25 and rw<=0.025:
                    ...
                    if angular_z_speed <= 0.160: # 0.05
                        stable_stop_count += 1  # Increment the counter

...
class MarkerVisualization(Node):
    ...
    def test_collision_object(self):
        ...
        col_marker1.pose.position.z = self.pos_z - 0.030
        ...
        col_marker2.pose.position.z = self.pos_z + 0.075
        ...
        pregrasp_pose.pose.position.z = self.pos_z2 + 0.10 # Lift slightly to avoid dragging
        ...
        lift_pose.pose.position.z = self.pos_z2 + 0.10 + 0.10 # 10cm higher + 10cm offset
        ...
        # Use pose1 as the pose for the marker
        move_marker3.pose = lift_pose.pose

        # Set scale for the arrow (Length, Width, Height)
        move_marker3.scale.x = 0.3  # Length of arrow
        move_marker3.scale.y = 0.02 # Arrow head width
        move_marker3.scale.z = 0.02 # Arrow head height

        # Set color (Green)
        move_marker3.color.g = 1.0
        move_marker3.color.a = 1.0 # Opaque


        # Publish the marker so it shows in Rviz
        self.pub.publish(move_marker4)
        ...
        self.pub.publish(move_marker2)
        self.pub.publish(move_marker1)
        # create a list
 ...
 # OBJ1 is the ns/id of your object
    ...
    # --- PHASE 3: LIFT ---
        ...
        if plan_lift and plan_lift.success:
            ...
            self.get_logger().error("lift Sucess................")
        else:
   ...
   # close partialy left
    ...
    def call_execution_service(self, plans):
        ...
        try:
            ...
            if not self._wait_future(future, 30.0): # if execution take more than 30 sec it aborts
